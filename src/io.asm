section .text
    ...
    ;Leer_número desde la entrada
leer_numero:
    mov rdi, mensaje_num      ;mensaje ingresar número
    xor eax, eax              ;Limpiar eax antes de printf
    call printf               ;Llamar a printf mostrar mensaje

    mov rdi, format_num        ;Dirección para leer el número
    mov rsi, numero            ;Dirección variable donde guardar el número
    xor eax, eax               ;Limpiar eax antes de llamar a scanf
    call scanf                 ;Llamar a scanf para leer el número

    ;Limpiar el buffer de entrada para evitar caracteres no deseados (importante)
    call limpiar_buffer

    mov eax, [numero]          ;Cargar el número leído en eax (retorno)
    ret                        ;Retornar el número

    ;Leer la operación las cuales son (+, -, *, /, %)
leer_operacion:
